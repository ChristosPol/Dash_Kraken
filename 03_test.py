...
trades_message_ready = json.dumps(trades_message)
book_message_ready = json.dumps(book_message)


def ws_open_trades(ws):
    print("✅ WebSocket connection opened for trades.")
    ws.send(trades_message_ready)

def ws_open_book(ws):
    print("✅ Kraken V2 WebSocket connection opened.")
    ws.send(book_message_ready)



def ws_message_trades(ws, message):
...
def run_websocket_trades():
    ws = websocket.WebSocketApp('wss://ws.kraken.com/v2', on_open=ws_open_trades, on_message=ws_message_trades)
    ws.run_forever()

def run_websocket_book():
    ws = websocket.WebSocketApp('wss://ws.kraken.com/v2', on_open=ws_open_book, on_message=on_message)
    ws.run_forever()


ws_thread_trades = threading.Thread(target=run_websocket_trades, daemon=False)
ws_thread_trades.start()
